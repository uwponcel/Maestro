...
namespace Maestro.Services.Community
{
    public class CommunityApiClient : IDisposable
    {
        ...
        private static string NamespaceSegment(SongNamespace ns)
        {
            switch (ns)
            {
                case SongNamespace.Builtin:
                    return "builtin";
                case SongNamespace.Community:
                    return "community";
                case SongNamespace.CommunityPending:
        ...
        public async Task<CommunityManifest> FetchManifestAsync(SongNamespace ns, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                Logger.Info($"Fetched {ns} manifest with {manifest?.Songs?.Count ?? 0} songs");
                return manifest;
            }
        ...
        public async Task<Song> FetchSongAsync(SongNamespace ns, string songId, CancellationToken cancellationToken = default)
        {
            try
            {
                var url = $"{STATIC_HOST_BASE}/{NamespaceSegment(ns)}/songs/{songId}.json";
