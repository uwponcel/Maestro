...
namespace Maestro
{
    ...
    public class Module : Blish_HUD.Modules.Module
    {
        ...
        protected override async Task LoadAsync()
        {
            ...
            const string debugSongsPath = @"C:\git\perso\Maestro\module\Songs";
            var isDebugMode = Directory.Exists(debugSongsPath);
            ...
            var storedSongs = _songStorage.GetAllSongs();
            ...
            _communityService.BuiltInSongSynced += (s, song) => MaybeRefreshAfterBuiltInSync();
            _communityService.BuiltInSyncFailed += (s, e) =>
                ScreenNotification.ShowNotification("Failed to load built-in songs", ScreenNotification.NotificationType.Error);

            if (!isDebugMode)
            {
                Logger.Info("Production mode: syncing built-in songs from static hosting");
                _ = SyncBuiltInSongsAndRefreshAsync();
            }
        ...
        private static readonly TimeSpan BuiltInRefreshThrottle = TimeSpan.FromMilliseconds(250);
        ...
        protected override void Unload()
        {
            ...
            // Null out (not just Dispose) before cancelling the sync below: a built-in sync
            // may already have queued a GameService.Overlay.QueueMainThreadUpdate closure that
            // fires _maestroWindow?.RefreshAfterCommunityDownload() on some future frame - that
            // queue is owned by Blish HUD core, not CommunityService, so Dispose()/cancellation
            // here cannot un-queue it. Nulling makes any such stray call a safe no-op instead of
            // a live call into a disposed window.
            _maestroWindow = null;
