...
namespace Maestro.UI.Community
{
    public class UploadWindow : StandardWindow
    {
                ...
                new Rectangle(15, MaestroTheme.WindowContentTopPadding, Layout.ContentWidth, Layout.ContentHeight))
        {
            ...
            _uploadableSongs = songs
                .Where(s => (s.IsUserImported || s.IsCreated) && !s.IsCommunityDownloaded)
                .ToList();
        ...
        private void BuildUi()
        {
            ...
            {
                _songSelector.Items.Add("No songs available");
                _songSelector.SelectedItem = "No songs available";
            ...
            else
            {
                ...
                {
                    _songSelector.Items.Add($"{song.Name} - {song.Artist}");
                }
                _songSelector.SelectedItem = _songSelector.Items.FirstOrDefault();
            }
        ...
        {
            var index = _songSelector.Items.ToList().IndexOf(_songSelector.SelectedItem);
            if (index >= 0 && index < _uploadableSongs.Count)
            {
                _selectedSong = _uploadableSongs[index];
                UpdateSongDetails();
                UpdateValidation();
            }
        }
        ...
        private async void OnUploadClicked(object sender, MouseEventArgs e)
        {
            ...
            try
            {
                ...
                if (response.Success)
                {
                    ScreenNotification.ShowNotification($"Upload successful! PR created.", ScreenNotification.NotificationType.Info);
